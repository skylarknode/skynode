define([
		...
		if (notifData.path) {
			...
			payload.clickfn = function () {
				...
				if (notifData.path.startsWith('http') || notifData.path.startsWith('https')) {
					...
					window.location.href = window.location.protocol + '//' + window.location.host + app.pageBasePath + notifData.path;
				}
			};
		} else {
	...
	function scrollToPostIndexIfOnPage(notifEl) {
		...
		if (path.startsWith(app.pageBasePath + '/post/') && pid && postEl.length && ajaxify.data.template.topic) {
