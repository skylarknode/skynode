define([],function() {
	var ioParams = {
		...
		path: app.apiBaseUrl + '/socket.io',
        //TODO: temporarily modified by lwf for test cors
    	withCredentials: true
	};

    //TODO: temporarily modified by lwf for test cors
    //var socket = window.socket = io(config.websocketAddress, ioParams);
	///var socket = window.socket = io("localhost:5567", ioParams);
	var socket = window.socket = io(app.apiBaseUrl, ioParams);
	...
	socket.init = function(app) {
		...
		function reJoinCurrentRoom() {
			...
			var	url_parts = window.location.pathname.slice(app.pageBasePath.length).split('/').slice(1);
			var room;
		...
		function onEventBanned(data) {
			...
			bootbox.alert({
				...
				callback: function () {
					...
					window.location.href = app.pageBasePath + '/';
				},
			});
		}
