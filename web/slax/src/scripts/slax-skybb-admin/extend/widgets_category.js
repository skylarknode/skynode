define([
	...
	'slax-skybb-base/topics/categorySelector'

], function ($,jqueryui,socket,categorySelector) {
	'use strict';
	var Widgets = {};
	var cid;
	...
	Widgets.init = function () {
		...
		categorySelector.init($('[component="category-selector"]'), function (category) {
			...
			ajaxify.go('admin/extend/widgets/category/' + (cid || ''));
		});
	...
	function prepareWidgets() {
		...
		}).on('mouseup', '.panel > .panel-heading', function (evt) { //modified by lwf
		...
		function saveWidgets() {
			if (!cid) {
				app.alertError("Please select a category!");
				return;
			}
			var saveData = {
				cid,
			...
			socket.emit('admin.widgets.categories.set', saveData, function (err) {
				if (err) {
					app.alertError(err.message);
				}

				app.alert({
					alert_id: 'admin_widgets', // 'admin:widgets', // modified by lwf
					type: 'success',
	...
	function loadWidgetData() {
		...
		$.get(RELATIVE_PATH + '/api/admin/extend/widgets/category/'+cid, function (data) {
