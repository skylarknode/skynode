define([
	...
	function addPostHandlers(tid) {
		...
		$('.topic').on('click', '[component="topic/reply-as-topic"]', function () {
			...
			translator.translate('[[topic:link_back, ' + ajaxify.data.titleRaw + ', ' + app.pageBasePath + '/topic/' + ajaxify.data.slug + ']]', function (body) {
	...
	function showStaleWarning(callback) {
		return callback();  // don't check timestamp. modified by lwf d
				...
				buttons: {
					...
					create: {
						...
						callback: function () {
							...
							translator.translate('[[topic:link_back, ' + ajaxify.data.title + ', ' + app.pageBasePath + '/topic/' + ajaxify.data.slug + ']]', function (body) {
								$(window).trigger('action:composer.topic.new', {
									cid: ajaxify.data.cid,
									topicType : ajaxify.data.topicType,
									body: body,
