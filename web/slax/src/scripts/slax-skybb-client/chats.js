define([
		...
		buttonEl.on('click', function () {
			Benchpress.parse('partials/modals/manage_room', {}, function (html) {
				translator.translate(html, function (html) {
					...
					//require(['autocomplete', "slax-skybb-base/misc/translator"], function (autocomplete, translator) {
	...
	Chats.switchChat = function (roomid) {
		...
		if (self.fetch) {
			...
			fetch(app.apiBaseUrl + '/api/' + url, { credentials: 'include' })
						...
						response.json().then(function (payload) {
							app.parseAndTranslate('partials/chats/message-window', payload, function (html) {
								...
								if (history.pushState) {
									...
									}, null, app.pageBaseurl + '/user/' + payload.userslug + '/chats/' + payload.roomId);
								}
							});
						});
					} else {
	...
	recentChats.switchChat = Chats.switchChat; // by lwf
	searchChats.switchChat = Chats.switchChat; // by lwf
