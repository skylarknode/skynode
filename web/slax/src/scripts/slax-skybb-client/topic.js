define([
	...
	'slax-skybb-base/misc/storage',
	'slax-skybb-base/misc/infinitescroll',
	"slax-skybb-base/misc/search",
	'./topic/threadTools',
	'./topic/postTools',
	'./topic/events',
	'./topic/posts',
	...
	function addCommentHandlers() {
		$('[component="post"]').each(function(idx,elm){
			var $post = $(elm),
				pid = $post.attr('data-pid'),
				$inputEl = $post.find(".comment-input"),
				$sendEl = $post.find(".comment-send");

			CommentCreate.init($inputEl);

			$sendEl.off('click').on('click', function () {
				CommentCreate.send($inputEl);
				$inputEl.focus();
				return false;
			});
		});
