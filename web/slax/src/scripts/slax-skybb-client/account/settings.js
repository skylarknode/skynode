define( [
	...
	'./header'
], function ($, socket, components, sounds, translator,header) {
	'use strict';
	var	AccountSettings = {};

	// If page skin is changed but not saved, switch the skin back
	$(window).on('action:ajaxify.start', function () {
		if (ajaxify.data && ajaxify.data.template && ajaxify.data.template.name === 'account/settings' && $('#bootswatchSkin').val() !== config.bootswatchSkin) {
	...
	AccountSettings.init = function () {
		...
		$('#submitBtn').on('click', function () {
			...
			if (settings.homePageRoute === 'custom' && settings.homePageCustom) {
				...
				$.get(app.apiBaseUrl + '/' + settings.homePageCustom, function () {
