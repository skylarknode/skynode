		...
		components.get('user/sessions').on('click', '[data-action]', function () {
			...
			if (uuid) {
				...
				$.ajax({
					...
					url: app.apiBaseUrl + '/api/user/' + ajaxify.data.userslug + '/session/' + uuid,
					method: 'delete',
					headers: {
					...
					try {
						...
						if (errorObj.loggedIn === false) {
							...
							window.location.href = app.pageBasePath + '/login?error=' + errorObj.title;
						}
