define([
	...
	'./topicList',
	'slax-skybb-base/misc/sort',
	'./category/tools',
	'slax-skybb-base/misc/infinitescroll',
], function ($, socket, utils,share, navigator, topicList, sort, categoryTools,infinitescroll) {
	'use strict';
	...
	Category.navigatorCallback = function (topIndex, bottomIndex) {
	    ...
	    var newUrl = 'category/' + ajaxify.data.slug;

	    if (newUrl !== currentUrl) {
	      if (Category.replaceURLTimeout) {
	        clearTimeout(Category.replaceURLTimeout);
	      }

	      Category.replaceURLTimeout = setTimeout(function () {
	        Category.replaceURLTimeout = 0;
	        if (history.replaceState) {
	          ...
	          }, null, window.location.protocol + '//' + window.location.host + app.pageBasePath + '/' + newUrl + search);
	        }
	        currentUrl = newUrl;
	      }, 500);
	    }
