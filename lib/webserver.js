...
var helpers = require('./helpers'); // modified by lwf
...
server.on('error', function (err) {
	if (err.code === 'EADDRINUSE') {
		winston.error('SkyBB address in use, exiting...', err);
	} else {
		winston.error(err);
	}
...
module.exports.listen = function (callback) {
	...
	async.waterfall([
		...
		function (next) {
			...
			initializeSkyBB(next);
		},
		function (next) {
			winston.info('SkyBB Ready');
...
function initializeSkyBB(callback) {
...
function listen(callback) {
	...
	args.push(function (err) {
		if (err) {
			winston.info('[startup] SkyBB was unable to listen on: ' + bind_address + ':' + port);
			process.exit();
		}

		winston.info('SkyBB is now listening on: ' + (isSocket ? socketPath : bind_address + ':' + port));
		if (oldUmask) {
	...
	if (isSocket) {
		...
		module.exports.testSocket(socketPath, function (err) {
			if (err) {
				winston.error('[startup] SkyBB was unable to secure domain socket access (' + socketPath + ')', err);
				throw err;
			}
