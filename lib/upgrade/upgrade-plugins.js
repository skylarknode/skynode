...
function getInstalledPlugins(callback) {
	async.parallel({
		...
		var isNbbModule = /^(nodebb|skybb)-(?:plugin|theme|widget|rewards)-[\w-]+$/;//modified by lwf
		var checklist;
	...
	checkPlugins(standalone, function (err, found) {
		...
		prompt.get({
			...
			if (['y', 'Y', 'yes', 'YES'].includes(result.upgrade)) {
				...
				console.log('Package upgrades skipped'.yellow + '. Check for upgrades at any time by running "'.reset + './skybb upgrade -p'.green + '".'.reset);
				callback();
			}
		});
	});
