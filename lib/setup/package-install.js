...
function updatePackageFile() {
	...
	var defaultPackageContents = require("../meta/install/package.json");
...
function installAll() {
	...
	try {
		...
		var packageManager = system.require('skynode-basis/system/configures').get('package_manager');
...
function preserveExtraneousPlugins() {
	...
	var isPackage = /^(nodebb|skybb)-(plugin|theme|widget|reward)-\w+/;//modified by lwf
