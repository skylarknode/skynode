	...
	async.parallel({
		tags: function (next) {
			var defaultTags = [{
				...
				content: Meta.config.title || 'SkyBB',
			}, {
		...
		links: function (next) {
			...
			if (plugins.hasListeners('filter:search.query')) {
				defaultLinks.push({
					...
					title: utils.escapeHTML(String(Meta.config.title || Meta.config.browserTitle || 'SkyBB')),
		...
		addIfNotExists(meta, 'property', 'og:title', Meta.config.title || 'SkyBB');
