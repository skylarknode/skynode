...
var pubsub = require('skynode-basis/pubsub');
...
function processConfig(data, callback) {
	async.parallel([
		...
		function (next) {
			var image = require('skynode-basis/image');
			if (data['brand:logo']) {
				image.size(nfs.join(nconf.get('upload_path'), 'system', 'site-logo-x50.png'), function (err, size) {
