...
var events = system.require('skynode-basis/events');
...
Themes.get = function (callback) {
	...
	var themePath = nfs.join(nconf.get('base_dir'), 'node_modules');
	...
	async.waterfall([
		function (next) {
			nfs.readdir(themePath, next);
		},
		function (dirs, next) {
			async.map(dirs.filter(function (dir) {
				return themeNamePattern.test(dir) || dir.startsWith('@');
			}), function (dir, next) {
				var dirpath = nfs.join(themePath, dir);

				nfs.stat(dirpath, function (err, stat) {
					if (err) {
						if (err.code === 'ENOENT') {
					...
					nfs.readdir(dirpath, function (err, themes) {
						if (err) {
							return next(err);
						}
						...
						async.filter(themes.filter(function (theme) {
							...
							nfs.stat(nfs.join(dirpath, theme), function (err, stat) {
								if (err) {
									if (err.code === 'ENOENT') {
										return next(null, false);
									}
		...
		function (themes, next) {
			...
			async.map(themes, function (theme, next) {
				var config = nfs.join(themePath, theme, 'theme.json');

				nfs.readFile(config, 'utf8', function (err, file) {
					if (err) {
						if (err.code === 'ENOENT') {
							return next(null, null);
						}
...
Themes.set = function (data, callback) {
	...
	case 'local':
		async.waterfall([
			...
			function (next) {
				nfs.readFile(nfs.join(nconf.get('themes_path'), data.id, 'theme.json'), 'utf8', function (err, config) {
					if (!err) {
			...
			function (config, next) {
				...
				// Re-set the themes path (for when SkyBB is reloaded)
...
Themes.setPath = function (themeObj) {
	...
	var themes_path = nfs.join(nconf.get('base_dir'), 'node_modules');
	...
	var themePath;
	var fallback = nfs.join(themes_path, themeObj.id, 'templates');

	if (themeObj.templates) {
		themePath = nfs.join(themes_path, themeObj.id, themeObj.templates);
	} else if (nfs.existsSync(fallback)) {
		themePath = fallback;
	}

	nconf.set('theme_templates_path', themePath);
	nconf.set('theme_config', nfs.join(themes_path, themeObj.id, 'theme.json'));
};
