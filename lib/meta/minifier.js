...
function minifyJS_batch(data, callback) {
	async.eachLimit(data.files, 100, function (fileObj, next) {
		nfs.readFile(fileObj.srcPath, 'utf8', function (err, source) {
			if (err) {
				return next(err);
			}
...
function minifyAndSave(data, callback) {
	...
	async.parallel([
		async.apply(nfs.writeFile, data.destPath, minified.code),
		async.apply(nfs.writeFile, data.destPath + '.map', minified.map),
	], callback);
}
