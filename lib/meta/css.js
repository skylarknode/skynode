...
function getBundleMetadata(target, callback) {
	var paths = [
		...
		nfs.join(nconf.get('base_dir'), 'slax/src/skins/less'),
	...
	async.waterfall([
		function (next) {
			if (target === 'client') {
				...
				nfs.rmdir(nfs.join(nconf.get('base_dir'), 'build/public/client*'), nfs. next);
			} else {
				setImmediate(next);
			}
		},
		...
		function (bundle, next) {
			...
			nfs.writeFile(nfs.join(nconf.get('base_dir'), 'build/public', filename), bundle.code, function (err) {
